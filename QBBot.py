...
async def on_message(text):
    ...
    if text.author.bot == False:
        ...
        if isInt(text.content): #Assigns points. Checks if bot because, otherwise, the Bot's own embedded messages would throw errors.
        ...
        if text.content.startswith('!help') or text.content.startswith('!commands') or text.content.startswith('!tutorial'):
            ...
            emb.add_field(name= "Any positive or negative whole number", value= "After a buzz or a bonus, the reader can enter a whole number to assign points.", inline=False)
            ...
            emb.add_field(name= "!bonusmode", value= "Toggles if bonuses are enabled. They are enabled by default.", inline=False)
            emb.add_field(name= "!bstop", value= "Kills an active bonus without giving points.", inline=False)
            await text.channel.send(embed=emb)
